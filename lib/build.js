...
function build(gyp, argv, callback) {
    ...
    compile.run_gyp(gyp_args.concat(command_line_args),opts,function(err,gopts) {
        // springmeyer: hmm, probably right inside here
        return callback(err);
        // springmeyer: that is called when the build is finished
        // springmeyer: node-pre-gyp supports chaining commands
        // springmeyer: so you might want to trigger an auto-package
        // springmeyer: which would call  `lib/package.js`
    });
}
