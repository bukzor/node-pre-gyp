...
function place_binary(from,to,opts,callback) {
    // springmeyer: so perhaps better to put the cache check right here before the download is attempted
    download(from,opts,function(err,req) {
        if (err) return callback(err);
        if (!req) return callback(new Error("empty req"));
        var badDownload = false
            , extractCount = 0
            // buck1: hopefully the tarball-unpacking function is something i can re-use from the s3 case
            // springmeyer: yes, should be doable
            // springmeyer: the gunzip/extracter expect a stream
            // springmeyer: in this download case it is the `req.pipe` that streams from http but a file stream would work too
            // springmeyer: an example of opening a tarball from the filesystem is at
            // springmeyer: https://github.com/mapbox/node-pre-gyp/blob/master/lib/testpackage.js
        ...
        req.on('response', function (res) {
            ...
            // springmeyer: this is an example of auto-triggering another command:
...
function install(gyp, argv, callback) {
    ...
    if (should_do_source_build) {
        ...
        // springmeyer: you can take advantage of this line
        // springmeyer: basically it checks if the binary is already in the right/expected place
        if (existsAsync(binary_module,function(found) {
            if (found) {
                ...
                // springmeyer: I think you can just step in front of this line
                // springmeyer: hmm, actually `place_binary` is called from two places...
                place_binary(from,to,opts,function(err) {
